...
class Point {
    ...
    setLeft(x, y) {
        this.isplain = false;
        this.left = new Coord(x, y);
    }

    setRight(x, y) {
        this.isplain = false;
        this.right = new Coord(x, y);
    }
    ...
    getLeft() {
        return this.isplain ? this.main : this.left == null ? this.main : this.left;
    }

    getRight() {
        return this.isplain ? this.main : this.right == null ? this.main : this.right;
    }
}
    ...
    toSVG() {
        ...
        for (let p = 1; p < points.length; p++) {
            ...
            if (curr.isPlain() && prev.isPlain()) {
                let lx = curr.getPoint().x;
                let ly = curr.getPoint().y;
                svg += "L" + lx + (ly < 0 ? ly : " " + ly);
            } else {
                let cx1 = prev.getRight().x;
                let cy1 = prev.getRight().y;
                let cx2 = curr.getLeft().x;
                let cy2 = curr.getLeft().y;
                let x2 = curr.getPoint().x;
                let y2 = curr.getPoint().y;
...
class Outline {
    ...
    getShapes() {
        return this.curveshapes;
    }

    getShape(index) {
        return this.curveshapes[index];
    }
...
class SVGParser {
        ...
        for (let s = 1; s < strings.length; s++) {
            ...
            if (op === "m" || op === "M") {
                if (op === "m") {
                    ...
                    x = parseFloat(terms[0]) + xoffset;
                    y = parseFloat(terms[1]) + yoffset;
                }
            ...
            // line instructions
                ...
                for (let t = 0; t < terms.length; t += 2) {
                    if (op === "l") {
                        x += parseFloat(terms[t + 0]);
                        y += parseFloat(terms[t + 1]);
                    } else if (op === "L") {
                        x = parseFloat(terms[t + 0]) + xoffset;
                        y = parseFloat(terms[t + 1]) + yoffset;
                    }
                    receiver.addPoint(x, y);
                }
            }
            ...
            // quadratic curve instruction
                ...
                for (let q = 0; q < terms.length; q += 4) {
                    ...
                    let cy = 0;
                    if (op === "q") {
                        cx = x + parseFloat(terms[q]);
                        cy = y + parseFloat(terms[q + 1]);
                    } else if (op === "Q") {
                        cx = parseFloat(terms[q]) + xoffset;
                        cy = parseFloat(terms[q + 1]) + yoffset;
                    }
                    ...
                    // set correct bounds
                    const bounds = this.getQuadraticCurveBounds(x, y, parseFloat(terms[0]), parseFloat(terms[1]), parseFloat(terms[2]), parseFloat(terms[3]));
                    receiver.updateBounds(bounds.xMin, bounds.yMin, bounds.xMax, bounds.yMax);
                    ...
                    if (op === "q") {
                        x += parseFloat(terms[q + 2]);
                        y += parseFloat(terms[q + 3]);
                    } else if (op === "Q") {
                        x = parseFloat(terms[q + 2]) + xoffset;
                        y = parseFloat(terms[q + 3]) + yoffset;
                    }
                    ...
                    receiver.addPoint(x, y);
                    if (x != cx2 || y != cy2) {
                        receiver.setLeftControl(cx2, cy2);
                    }
                }
            }
            ...
            // cubic curve instruction
                ...
                for (let c = 0; c < terms.length; c += 6) {
                    ...
                    let cy1 = 0;
                    if (op === "c") {
                        cx1 = x + parseFloat(terms[c]);
                        cy1 = y + parseFloat(terms[c + 1]);
                    } else if (op === "C") {
                        cx1 = parseFloat(terms[c]) + xoffset;
                        cy1 = parseFloat(terms[c + 1]) + yoffset;
                    }
                    ...
                    let cy2 = 0;
                    if (op === "c") {
                        cx2 = x + parseFloat(terms[c + 2]);
                        cy2 = y + parseFloat(terms[c + 3]);
                    } else if (op === "C") {
                        cx2 = parseFloat(terms[c + 2]) + xoffset;
                        cy2 = parseFloat(terms[c + 3]) + yoffset;
                    }

                    // NOTE: the relative cubic instruction does not count control points as "real"
                    // points, so the curve's on-curve coordinate is relative to the last on-curve one.
                    if (op === "c") {
                        x += parseFloat(terms[c + 4]);
                        y += parseFloat(terms[c + 5]);
                    } else if (op === "C") {
                        x = parseFloat(terms[c + 4]) + xoffset;
                        y = parseFloat(terms[c + 5]) + yoffset;
                    }
    ...
    getQuadraticCurveBounds(ax, ay, bx, by, cx, cy) {
        function P(x, y) {
            this.x = x;
            this.y = y;
        }
        ...
        function pointOnCurve(P1, P2, P3, t) {
            ...
            const c1 = new P(P1.x + (P2.x - P1.x) * t, P1.y + (P2.y - P1.y) * t);
            const c2 = new P(P2.x + (P3.x - P2.x) * t, P2.y + (P3.y - P2.y) * t);

            return new P(c1.x + (c2.x - c1.x) * t, c1.y + (c2.y - c1.y) * t);
        }

        const P1 = new P(ax, ay);
        const P2 = new P(bx, by);
        const P3 = new P(cx, cy);

        const tx = (P1.x - P2.x) / (P1.x - 2 * P2.x + P3.x);
        const ty = (P1.y - P2.y) / (P1.y - 2 * P2.y + P3.y);

        const Ex = pointOnCurve(P1, P2, P3, tx);
        const xMin = Ex ? Math.min(P1.x, P3.x, Ex.x) : Math.min(P1.x, P3.x);
        const xMax = Ex ? Math.max(P1.x, P3.x, Ex.x) : Math.max(P1.x, P3.x);

        const Ey = pointOnCurve(P1, P2, P3, ty);
        const yMin = Ey ? Math.min(P1.y, P3.y, Ey.y) : Math.min(P1.y, P3.y);
        const yMax = Ey ? Math.max(P1.y, P3.y, Ey.y) : Math.max(P1.y, P3.y);
...
module.exports = {Outline, SVGParser, SVGPath};